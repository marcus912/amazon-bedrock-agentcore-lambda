...
class TestInvokeAgent:
    ...
    @patch('integrations.agentcore_invocation.bedrock_client')
    def test_invoke_agent_throttling(self, mock_bedrock_client):
...
class TestInvokeAgentAsync:
    """Test the invoke_agent_async function (fire-and-forget)."""

    @patch('integrations.agentcore_invocation.bedrock_client')
    def test_invoke_agent_async_success(self, mock_bedrock_client):
        """Test successful async agent invocation (does not read response)."""
        from integrations import agentcore_invocation

        mock_response = {
            'response': MagicMock()  # Response object that should NOT be read
        }
        mock_bedrock_client.invoke_agent_runtime.return_value = mock_response

        # Execute
        result = agentcore_invocation.invoke_agent_async(
            prompt="Create GitHub issue for this bug"
        )

        # Assert
        assert result.startswith("Agent invoked asynchronously: session_id=")
        assert "session-" in result
        mock_bedrock_client.invoke_agent_runtime.assert_called_once()

        # CRITICAL: Verify response was NOT read (async behavior)
        mock_response['response'].read.assert_not_called()

    @patch('integrations.agentcore_invocation.bedrock_client')
    def test_invoke_agent_async_with_session_id(self, mock_bedrock_client):
        """Test async invocation with custom session ID."""
        from integrations import agentcore_invocation

        mock_response = {'response': MagicMock()}
        mock_bedrock_client.invoke_agent_runtime.return_value = mock_response

        custom_session = "session-" + "a" * 30
        result = agentcore_invocation.invoke_agent_async(
            prompt="Follow up",
            session_id=custom_session
        )
    ...
    def test_invoke_agent_async_empty_prompt(self):
        """Test that ValidationException is raised for empty prompt."""
        from integrations import agentcore_invocation

        with pytest.raises(agentcore_invocation.ValidationException, match="non-empty string"):
            agentcore_invocation.invoke_agent_async(prompt="")

    def test_invoke_agent_async_none_prompt(self):
        """Test that ValidationException is raised for None prompt."""
        from integrations import agentcore_invocation

        with pytest.raises(agentcore_invocation.ValidationException, match="non-empty string"):
            agentcore_invocation.invoke_agent_async(prompt=None)

    def test_invoke_agent_async_short_session_id(self):
        """Test that ValidationException is raised for short session ID."""
        from integrations import agentcore_invocation
    ...
    @patch('integrations.agentcore_invocation.bedrock_client')
    def test_invoke_agent_async_resource_not_found(self, mock_bedrock_client):
        """Test AgentNotFoundException when agent doesn't exist."""
        from integrations import agentcore_invocation
        ...
        with pytest.raises(agentcore_invocation.AgentNotFoundException, match="Agent not found"):
            agentcore_invocation.invoke_agent_async(prompt="Test prompt")

    @patch('integrations.agentcore_invocation.bedrock_client')
    def test_invoke_agent_async_throttling(self, mock_bedrock_client):
        """Test throttling error in async invocation."""
        from integrations import agentcore_invocation
    ...
    @patch('integrations.agentcore_invocation.bedrock_client')
    def test_invoke_agent_async_no_response_read(self, mock_bedrock_client):
        """
        CRITICAL TEST: Verify that async invocation does NOT read the response stream.
        This allows Lambda to return immediately while agent continues processing.
        """
        from integrations import agentcore_invocation
        ...
        result = agentcore_invocation.invoke_agent_async(prompt="Test prompt")

        # CRITICAL ASSERTION: Response stream should NEVER be read
        mock_response_stream.read.assert_not_called()
        assert "Agent invoked asynchronously" in result
